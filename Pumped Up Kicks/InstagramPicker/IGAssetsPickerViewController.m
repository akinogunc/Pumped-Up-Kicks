...
- (void)loadPhotos {
    ...
    if (status == PHAuthorizationStatusAuthorized)
    {
        ...
        PHFetchResult *allMedia = [PHAsset fetchAssetsWithMediaType:assetType options:self.fetchOptions];
        long mediaCount = [allMedia count];
        [allMedia enumerateObjectsUsingBlock:^(PHAsset *asset, NSUInteger idx, BOOL *stop) {
            if (asset && asset.mediaSubtypes != PHAssetMediaSubtypeVideoHighFrameRate) {//exclude slow mo
    ...
    else
    {
        [PHPhotoLibrary requestAuthorization:^(PHAuthorizationStatus status) {
            if (status != PHAuthorizationStatusAuthorized){
                ...
                UIAlertController * alertController = [UIAlertController alertControllerWithTitle:@"App can't access photos" message:@"To give permission tap on 'Change Settings' button" preferredStyle:UIAlertControllerStyleAlert];
...
//#ifdef IG_CROP_IMMEDIATELY
    ...
    {
        [self.cropView cropAssetandCallback:^(NSDictionary* result){
            
            [self.delegate IGAssetsPickerFinishCroppingToAsset:result];
